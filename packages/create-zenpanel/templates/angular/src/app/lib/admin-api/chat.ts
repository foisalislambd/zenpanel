...
function buildDashboardReply(data: Record<string, unknown>, prompt: string): string {
  ...
  if (/growth|analyze|metric|stat|revenue|order|user|message/.test(lower)) {
    ...
    const revenueText = typeof revenue === 'number' ? formatCurrency(revenue) : '—';
...
2. Replace \`previewSendAdminChatMessage\` in \`src/app/lib/admin-api/chat.ts\`
3. Optionally register page context via the chat panel service`;
  }
...
export async function previewSendAdminChatMessage(input: {
  ...
  if (page && isDashboardPage(page.pageId) && page.data && Object.keys(page.data).length > 0) {
