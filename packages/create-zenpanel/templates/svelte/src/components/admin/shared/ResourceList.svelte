<script lang="ts">
  ...
  import { Database, MoreHorizontal, Pencil, Plus, Trash2 } from "lucide-svelte";
  ...
  let { items, resourceLabel }: Props = $props();
</script>
        ...
        <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
          {#each items as item (item.id)}
            <tr class="transition-colors hover:bg-gray-50/80 dark:hover:bg-white/[0.02]">
              ...
              <td class="px-6 py-4">
                <span class="inline-flex rounded-full px-2.5 py-0.5 text-xs font-medium capitalize {statusStyles[item.status]}">
                  {item.status}
                </span>
              </td>
              ...
              <td class="px-6 py-4">
                <div class="flex items-center justify-end gap-1">
                  <button type="button" disabled title={disabledActionTitle} class="flex h-8 w-8 cursor-not-allowed items-center justify-center rounded-lg text-gray-400" aria-label="Edit {item.title}">
                    <Pencil class="h-4 w-4" />
                  </button>
                  <button type="button" disabled title={disabledActionTitle} class="flex h-8 w-8 cursor-not-allowed items-center justify-center rounded-lg text-gray-400" aria-label="More actions for {item.title}">
                    <MoreHorizontal class="h-4 w-4" />
                  </button>
                  <button type="button" disabled title={disabledActionTitle} class="flex h-8 w-8 cursor-not-allowed items-center justify-center rounded-lg text-gray-400" aria-label="Delete {item.title}">
