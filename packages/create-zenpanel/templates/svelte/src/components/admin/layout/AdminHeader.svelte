<script lang="ts">
  ...
  import AdminUserMenu from "@/components/admin/layout/AdminUserMenu.svelte";
  ...
  import { Menu, PanelRightOpen, Search, X } from "lucide-svelte";
...
<header class="admin-topbar sticky top-0 z-30 w-full bg-white/95 backdrop-blur supports-[backdrop-filter]:bg-white/80 dark:bg-gray-900/95 dark:supports-[backdrop-filter]:bg-gray-900/80">
  <div class="flex h-full items-center gap-3 px-4 sm:gap-4 sm:px-6">
    <button
      ...
      class="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg border border-gray-200 text-gray-600 transition hover:bg-gray-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-500/30 dark:border-gray-800 dark:text-gray-400 dark:hover:bg-white/5 {sidebar.isDesktop ? 'lg:hidden' : ''}"
    ...
    >
      {#if !sidebar.isDesktop && sidebar.isMobileOpen}
        <X class="h-5 w-5" />
      {:else}
        <Menu class="h-5 w-5" />
      {/if}
    ...
    <RouterLink href="/admin" class="flex items-center gap-2 lg:hidden">
