<script lang="ts">
  import AdminChatMarkdown from "@/components/admin/chat/AdminChatMarkdown.svelte";
  import {
    CHAT_PANEL_WIDTH,
    useAdminChatPanel,
  ...
  const defaultQuickActions = $derived<AdminChatQuickAction[]>(
...
{#snippet messageBubble(message: AdminChatMessage)}
  {@const isUser = message.role === "user"}
  <div class={cn("admin-chat-bubble flex w-full min-w-0", isUser ? "justify-end" : "justify-start")}>
    <div
      class={cn(
...
{#if chat.isOpen}
  ...
  <aside
    class={cn(
      ...
      overlay || !chat.isDesktop ? "fixed top-0 right-0 z-50 h-dvh shadow-2xl" : "h-full shrink-0",
    )}
    style:width={chat.isDesktop && !overlay ? `${CHAT_PANEL_WIDTH}px` : "min(100vw, 400px)"}
    role={overlay || !chat.isDesktop ? "dialog" : undefined}
    aria-modal={overlay || !chat.isDesktop ? true : undefined}
    ...
    <div class="admin-scrollbar flex min-h-0 flex-1 flex-col overflow-y-auto px-4 py-4">
      {#if chat.messages.length > 0 || chat.isGenerating}
        <div class="space-y-4">
          ...
          {/if}
          <div bind:this={endRef} class="h-1"></div>
        </div>
      {:else}
