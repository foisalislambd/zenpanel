...
const error = ref("");
const themeIcon = ref<"sun" | "moon">("moon");
const username = ref(DEMO_USERNAME);
const password = ref(DEMO_PASSWORD);

function syncThemeIcon() {
  themeIcon.value = getTheme() === "dark" ? "sun" : "moon";
}
...
function onSubmit(e: Event) {
  e.preventDefault();
  error.value = "";
  const user = username.value.trim() || DEMO_USERNAME;
...
<template>
  <div class="login-page">
    <button
      ...
      :aria-label="
        themeIcon === 'sun' ? 'Switch to light mode' : 'Switch to dark mode'
      "
      @click="onToggleTheme"
    >
    ...
    <div class="login-form-wrap">
      <div class="login-form">
        ...
        <div style="margin-top: 2rem">
          <div class="brand-mark" style="margin-bottom: 1.5rem">
            {{ brand.letter }}
          </div>
          ...
          <p
            class="muted"
            style="margin: 0.5rem 0 0; font-size: 0.875rem; line-height: 1.5"
          >
            Preview UI — credentials are prefilled. Click sign in to open the
            dashboard.
          </p>
        </div>
        <form style="margin-top: 2rem" @submit="onSubmit">
          <div class="field">
        ...
        <p
          class="muted"
          style="margin-top: 2rem; text-align: center; font-size: 0.75rem"
        >
    ...
    <aside class="login-brand-panel">
      <div style="position: relative; z-index: 1; max-width: 28rem">
        ...
        <h2 style="margin: 1.5rem 0 0.75rem; font-size: 1.75rem">
          {{ brand.name }}
        </h2>
        ...
        >
          <li
            v-for="feature in brand.loginFeatures"
            :key="feature"
            style="
              display: flex;
              gap: 0.75rem;
