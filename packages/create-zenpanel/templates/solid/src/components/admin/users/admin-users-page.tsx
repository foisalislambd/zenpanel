...
function UserDetailPanel(props: { user: PortalUserRow; onClose: () => void }) {
  const rows = () => [
    { label: "Provider", value: props.user.authProvider, capitalize: true },
    { label: "Status", value: props.user.status, capitalize: true },
    {
      label: "Email verified",
      value: props.user.emailVerified ? "Yes" : "No",
      capitalize: false,
    },
    { label: "Country", value: dash(props.user.country), capitalize: false },
    { label: "Phone", value: dash(props.user.phone), capitalize: false },
    { label: "Last IP", value: dash(props.user.lastIp), capitalize: false },
    {
      label: "Joined",
      ...
      <aside class="relative flex h-full w-full max-w-md flex-col border-l border-gray-200 bg-white shadow-xl dark:border-gray-800 dark:bg-gray-900">
        <div class="flex items-center justify-between border-b border-gray-200 px-5 py-4 dark:border-gray-800">
          <div class="min-w-0">
            ...
            <p class="truncate text-sm text-gray-500">{props.user.email}</p>
          </div>
          <button
            type="button"
            onClick={props.onClose}
...
export function AdminUsersPage(props: AdminUsersPageProps) {
  ...
  const [providerFilter, setProviderFilter] = createSignal<ProviderFilter>("all");
  const [selected, setSelected] = createSignal<PortalUserRow | null>(null);
                ...
                <li class="px-4 py-3.5">
                  <div class="flex items-start justify-between gap-3">
                    ...
                    <AdminRowActions
                      ...
                      onEdit={props.onEdit ? () => props.onEdit?.(user) : undefined}
                      onDelete={props.onDelete ? () => props.onDelete?.(user) : undefined}
                    />
                  </div>
                </li>
              )}
                    ...
                    <tr class="transition-colors hover:bg-gray-50/80 dark:hover:bg-white/[0.02]">
                      ...
                      <td class="px-5 py-3.5">
                        <AdminRowActions
                          ...
                          onEdit={props.onEdit ? () => props.onEdit?.(user) : undefined}
                          onDelete={
                            props.onDelete ? () => props.onDelete?.(user) : undefined
                          }
                        />
                      </td>
                    </tr>
...
function StatusBadges(props: { user: PortalUserRow }) {
  return (
    <>
      <Show
        when={props.user.status === "active"}
        fallback={
          <span class="inline-flex rounded-full bg-error-50 px-2.5 py-0.5 text-xs font-medium text-error-500 dark:bg-error-500/15">
            Banned
          </span>
        }
      >
      ...
      <Show when={!props.user.emailVerified}>
        <span class="inline-flex rounded-full bg-orange-50 px-2.5 py-0.5 text-xs font-medium text-orange-700 dark:bg-orange-500/15 dark:text-orange-400">
          Unverified
        </span>
      </Show>
    </>
