...
function StatusBadge(props: { status: string }) {
  const style = () => statusStyles[props.status as OrderStatus] ?? fallbackStatusStyle;
  return (
    <span
      class={`inline-flex rounded-md px-2 py-0.5 text-xs font-medium capitalize ring-1 ring-inset ${style()}`}
    >
      {props.status}
    </span>
  );
...
export function RecentOrdersTable(props: { orders: RecentOrder[] }) {
  return (
